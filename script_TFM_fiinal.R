...
##Cargamos datos
datos_2018 <- read.csv(
  '/Users/laiamontesocardona/Desktop/TFM/UPDATE 10.07/script + datos 28.07/TFM_datos2018_definitivos.csv',
  sep = ";"
)
...
datos_2019 <- read.csv(
  '/Users/laiamontesocardona/Desktop/TFM/UPDATE 10.07/script + datos 28.07/TFM_datos2019_definitivos.csv',
  sep = ";"
)
str(datos_2019)
...
datos_2020 <- read.csv(
  '/Users/laiamontesocardona/Desktop/TFM/UPDATE 10.07/script + datos 28.07/TFM_datos2020_definitivos.csv',
  sep = ";"
)
...
data_captive <- read.csv('/Users/laiamontesocardona/Desktop/TFM/UPDATE 10.07/script + datos 28.07/TFM_datos_definitivos_numero_suelta.csv', sep = ";")
...
data_llibertat <- read_excel('/Users/laiamontesocardona/Desktop/TFM/UPDATE 10.07/script + datos 28.07/Captures llibertat.xlsx')
...
data <- read.csv('/Users/laiamontesocardona/Desktop/TFM/UPDATE 10.07/ara si/TFM_datos_definitivos_cautividad_libertad.csv', sep = ";")
str(data)
##Transformamos variables
...
ggplot(data_sex, aes(x = age_years, y = Rate_percent_years)) +
  ...
  ggplot(data_sex, aes(x = age_years, y = Rate_percent_years)) +
    ...
    theme_minimal() +
    theme(
      panel.background = element_blank(),       # Quita fondo gris
      plot.background = element_blank(),        # Quita fondo total
      strip.background = element_blank(),       # Quita fondo de facetas
      plot.title = element_text(size = 14, face = "bold"),
...
plot1 <- ggplot(data_sex, aes(age, W)) +
  ...
  ggtitle("Relación Peso y Edad - Figura 5a")
...
plot3 <- ggplot() +
  geom_point(data = data_females, 
             aes(x = age, y = L_C, color = "Hembras (cautividad)", alpha = 0.3)) + # Puntos hembras
  geom_point(data = data_females %>% filter(wildcaptures == "yes"),
             aes(x = age, y = L_C, color = "Hembras (recapturas libertad)"), alpha = 0.6) + # Puntos hembras
  geom_point(data = data_males, 
             aes(x = age, y = L_C, color = "Machos (cautividad)", alpha = 0.3)) +   # Puntos machos
  geom_point(data = data_males %>% filter(wildcaptures == "yes"),
             aes(x = age, y = L_C, color = "Machos (recapturas libertad)"), alpha = 0.6) +   # Puntos machos
  ...
  scale_color_manual(
    ...
    breaks = colores_ordenados  # Para controlar el orden exacto
  ) +
  guides(alpha = "none") +  # Oculta la leyenda del alpha
  theme_minimal() + 
...
ggplot() +
  geom_point(data = data_females, 
             aes(x = age, y = W, color = "Hembras (cautividad)", alpha = 0.3)) + # Puntos hembras
  geom_point(data = data_females %>% filter(wildcaptures == "yes"),
             aes(x = age, y = W, color = "Hembras (recapturas libertad)"), alpha = 0.6) + # Puntos hembras
  geom_point(data = data_males, 
             aes(x = age, y = W, color = "Machos (cautividad)", alpha = 0.3)) +   # Puntos machos
  geom_point(data = data_males %>% filter(wildcaptures == "yes"),
             aes(x = age, y = W, color = "Machos (recapturas libertad)"), alpha = 0.6) +   # Puntos machos
...
gtable <- growth_models_W %>%
  ...
  tab_options(
    table.font.names = "Arial",
    table.font.size = 13,        # tamaño de letra general
    data_row.padding = px(4),    # menos espacio entre filas
    heading.padding = px(6)
  ) %>%
  tab_style(
    style = cell_text(size = px(9)),   # referencias un poco más grandes
    locations = cells_body(columns = `Referencias del modelo`)
  ) %>%
  cols_width(
    Modelo ~ pct(35),
...
tabla_resumen <- data %>%
  ...
  mutate(stage_nombre = factor(stage_nombre, levels = c(
    "Acuario", "Tupper", "Depósito", "Cercado exterior", "Jaulas de aclimatación", "Suelta directa", "Suelta"
  ))) %>%
...
tabla_gt <- tabla_resumen %>%
  ...
  cols_label(
    stage = "Etapa",
    W_media = "W Media", W_mediana = "W Mediana", W_sd = "W SD", W_min = "W Min", W_max = "W Max", W_rango = "W Rango",
    L_C_media = "L_C Media", L_C_mediana = "L_C Mediana", L_C_sd = "L_C SD", L_C_min = "L_C Min", L_C_max = "L_C Max", L_C_rango = "L_C Rango"
  ) %>%
  tab_style(
...
getwd()
gtsave(tabla_gt, filename = "tabla_descriptivos_W_y_L_C.2.html")
